...
Ein GUI-Tool für Ubuntu/Linux zum Sichern und Zurückspielen von SD-Karten und
USB-Sticks — für Raspberry Pi OS, OpenWrt und praktisch jedes andere System.
...
- **Fail-closed statt fail-open** — Geräte ohne Seriennummer oder stabilen
  `by-id`-Pfad werden gar nicht erst zur Auswahl angeboten
...
![AuraPi Backup-Tab](screenshots/Backup.png)
...
![AuraPi Restore-Tab](screenshots/Restore.png)
...
Die Datei `aurapi_navy_theme.json` muss im selben Verzeichnis wie
`aurapi.py` liegen (enthält nur das Farbschema — Wortmarke und Icon sind
direkt im Code eingebettet, keine weiteren externen Dateien nötig).
